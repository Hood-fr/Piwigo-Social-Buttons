...
{else}
  <a title="Share on tumblr" href="https://www.tumblr.com/share/link?url={$SOCIALBUTT.share_url|urlencode}&name={$PAGE_TITLE|cat:' | '|cat:$GALLERY_TITLE|urlencode}&description={if !empty($CONTENT_DESCRIPTION)}{$CONTENT_DESCRIPTION|cat:$SOCIALBUTT.copyright|urlencode}"{else}{{$GALLERY_TITLE}|cat:$SOCIALBUTT.copyright|urlencode}"{/if}
{/if}
{if $SOCIALBUTT.light}
